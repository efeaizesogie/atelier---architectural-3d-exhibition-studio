...
function FocusedModelInteractionHandler({
  isFocusedMode,
}: {
  isFocusedMode: boolean;
}) {
    ...
    const handlePointerMove = (e: PointerEvent) => {
      if (isDragging.current) {
        ...
        window.dispatchEvent(
          new CustomEvent("rotate-focused-model", { detail: { deltaX } })
        );
      }
    };
    ...
    if (controlsRef.current) {
      if (isFocusedMode) {
        ...
        controlsRef.current.target.y = THREE.MathUtils.clamp(
          ...
          ROOM_BOUNDS.minY,
          ROOM_BOUNDS.maxY - 0.5
        );
        controlsRef.current.target.z = THREE.MathUtils.clamp(
          controlsRef.current.target.z,
          ROOM_BOUNDS.minZ + 0.5,
          ROOM_BOUNDS.maxZ - 0.5
        );
      }
    }
  });

...
export default function App() {
  ...
  const [exhibits, setExhibits] =
    useState<CharacterExhibit[]>(INITIAL_EXHIBITS);
  const [currentExhibit, setCurrentExhibit] = useState<CharacterExhibit>(
    INITIAL_EXHIBITS[0]
  );
  const [lightingConfig, setLightingConfig] =
    useState<LightingConfig>(DEFAULT_LIGHTING);
  const [materialConfig, setMaterialConfig] =
    useState<MaterialConfig>(DEFAULT_MATERIALS);
  const [optimizerSettings, setOptimizerSettings] =
    useState<ModelOptimizerSettings>(DEFAULT_OPTIMIZER);
  ...
  const [lockedExhibitId, setLockedExhibitId] = useState<string | null>(null);
  ...
  const [leftTab, setLeftTab] = useState<
    "lighting" | "materials" | "optimizer" | "none"
  >("none");
  ...
  const handleCycleWallMaterial = () => {
    const wallOptions: WallMaterialType[] = [
      "venetian_plaster",
      "raw_concrete",
      "walnut_slats",
      "limestone",
  ...
  const handleCycleFloorMaterial = () => {
    const floorOptions: FloorMaterialType[] = [
      "travertine",
      "microcement",
      "smoked_oak",
      "dark_basalt",
  ...
  // Toggle lock: clicking the same locked model unlocks it, clicking a different model locks to it
  const handleSelectExhibitById = (id: string) => {
    const found = exhibits.find((e) => e.id === id);
    if (found) {
      if (lockedExhibitId === id) {
        // Clicking the same locked model unlocks it
        setLockedExhibitId(null);
  ...
  // Handle exhibit selection & zoom focus into model space (locks camera to the model)
  const handleSelectExhibit = (ex: CharacterExhibit) => {
    setCurrentExhibit(ex);
    setLockedExhibitId(ex.id);
    setCameraPreset("hero");
  ...
  // Reset camera view and unlock from any focused model
  const handleResetView = () => {
    setLockedExhibitId(null);
    setCameraPreset("hero");
  };

  // Global Keyboard Listener: Press ESC or 'x'/'X' to reset camera & unlock from focused model
  React.useEffect(() => {
  ...
  const handleUploadGlbFile = (file: File) => {
    ...
    const newEx: CharacterExhibit = {
      ...
      description:
        "Uploaded 3D character asset processed through real-time mesh decimation and texture optimization pipeline.",
      curatorNote:
        "Positioned dynamically on a custom studio pedestal. Double-click to lock camera focus into character space.",
      type: "custom_glb",
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#171614] font-sans antialiased select-none">
      ...
      <StudioCanvas
        ...
        isFocusedMode={lockedExhibitId !== null}
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 flex items-center space-x-2 bg-stone-900/95 border border-stone-700/80 px-4 py-2 rounded-full shadow-2xl backdrop-blur-md pointer-events-auto transition-all">
        ...
        <button
          ...
          title="Reset camera & unlock from model (ESC)"
        >
          <X className="w-3.5 h-3.5" />
          <span>Reset View</span>
          <kbd className="text-[9px] bg-stone-800 px-1 rounded text-stone-300 border border-stone-700 ml-0.5">
            ESC
          </kbd>
        </button>
      </div>
      ...
      <div className="absolute top-4 right-4 z-30 flex items-center space-x-2 pointer-events-auto">
        <button
          ...
          title="Reset Camera View & Unlock"
        >
        ...
        <button
          ...
          title={
            isAudioPlaying ? "Mute Studio Acoustics" : "Enable Studio Acoustics"
          }
        >
          {isAudioPlaying ? (
        ...
        <>
          ...
          <div className="absolute top-16 left-6 z-20 flex flex-col space-y-2 pointer-events-auto max-h-[82vh] overflow-y-auto pr-1">
            <div className="bg-stone-900/90 border border-stone-800/80 p-1 rounded-lg flex space-x-1 backdrop-blur-md self-start shadow-xl">
              <button
                onClick={() =>
                  setLeftTab(leftTab === "lighting" ? "none" : "lighting")
                }
              ...
              <button
                onClick={() =>
                  setLeftTab(leftTab === "materials" ? "none" : "materials")
                }
                className={`px-3 py-1.5 rounded text-xs font-medium transition-all flex items-center space-x-1.5 ${
                  leftTab === "materials"
                    ? "bg-amber-500/20 text-amber-300 border border-amber-500/40 shadow-sm"
              ...
              <button
                onClick={() =>
                  setLeftTab(leftTab === "optimizer" ? "none" : "optimizer")
                }
                className={`px-3 py-1.5 rounded text-xs font-medium transition-all flex items-center space-x-1.5 ${
                  leftTab === "optimizer"
                    ? "bg-amber-500/20 text-amber-300 border border-amber-500/40 shadow-sm"
